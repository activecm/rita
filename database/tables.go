...
func (db *DB) createBigOlHistogramTable(ctx context.Context) error {
	if err := db.Conn.Exec(ctx, `--sql
		CREATE TABLE IF NOT EXISTS {database:Identifier}.big_ol_histogram (
			...
			hash FixedString(16),
			bucket DateTime(),
			src_ip_bytes SimpleAggregateFunction(sum, UInt64),
...
func (db *DB) createUconnTable(ctx context.Context) error {
	...
	CREATE TABLE IF NOT EXISTS {database:Identifier}.uconn (
		...
		src_ip_bytes_list AggregateFunction(groupArray(86400), UInt64),
		total_src_ip_bytes AggregateFunction(sum, UInt64),
		total_dst_ip_bytes AggregateFunction(sum, UInt64),
		total_src_bytes AggregateFunction(sum, UInt64),
		total_dst_bytes AggregateFunction(sum, UInt64),
		total_ip_bytes AggregateFunction(sum, UInt64),
		total_src_packets AggregateFunction(sum, UInt64),
		total_dst_packets AggregateFunction(sum, UInt64),
	...
	SELECT
		...
		groupArrayStateIf(86400)(c.src_bytes, missing_host_header = false) as src_ip_bytes_list,
...
func (db *DB) createUSNIConnTable(ctx context.Context) error {
	...
	SELECT
		...
		groupArrayState(86400)(s.src_bytes) as src_ip_bytes_list,
	...
	SELECT
		...
		uniqExactState(ts) as unique_ts_count,
		groupArrayState(86400)(toUnixTimestamp(ts)) as ts_list,
		groupArrayState(86400)(h.src_bytes) as src_ip_bytes_list,
...
func (db *DB) createDNSTable(ctx context.Context) error {
	err := db.Conn.Exec(ctx, `--sql
		CREATE TABLE IF NOT EXISTS {database:Identifier}.dns (
			...
			ttls Array(UInt32), -- TODO: should we change this to Array(Float64)?
			rejected Bool
		)
		ENGINE = MergeTree()
